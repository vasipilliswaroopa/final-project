...
import { userService } from "../services/api"; // <-- Line 3: Fixed path from "../services/api"
...
const LogIn: React.FC = () => {
  ...
  const handleLogIn = async (e: React.FormEvent) => {
    ...
    try {
      const res = await userService.login({ email: trimmedEmail, password: trimmedPassword });

      // <-- Fixed: Save token for ProtectedRoute interceptor
      if (res.token) {
        localStorage.setItem("token", res.token);
        localStorage.setItem("currentUser", JSON.stringify(res.user || res));
      } else {
